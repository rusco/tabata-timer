...
go$packages["main"] = (function() {
	var go$pkg = {}, js = go$packages["github.com/gopherjs/gopherjs/js"], jquery = go$packages["github.com/rusco/jquery"], math = go$packages["math"], strconv = go$packages["strconv"], TabataTimer, time, main, NewTabataTimer, jQuery;
	TabataTimer = go$pkg.TabataTimer = go$newType(0, "Struct", "main.TabataTimer", "TabataTimer", "main", function(element_, onSec_, offSec_, endDate_, interval_, rounds_, roundFinished_) {
		this.go$val = this;
		this.element = element_ !== undefined ? element_ : new jquery.JQuery.Ptr();
	...
	NewTabataTimer = go$pkg.NewTabataTimer = function() {
		var _struct, element, _struct$1, onSec, _struct$2, offSec, endDate, interval, rounds, roundFinished, _struct$3, _struct$4, _struct$5;
		element = (_struct = jQuery(new (go$sliceType(go$emptyInterface))([new Go$String("time")])), new jquery.JQuery.Ptr(_struct.o, _struct.Jquery, _struct.Selector, _struct.Length, _struct.Context));
		...
		rounds = 1;
		roundFinished = false;
		return new TabataTimer.Ptr((_struct$3 = element, new jquery.JQuery.Ptr(_struct$3.o, _struct$3.Jquery, _struct$3.Selector, _struct$3.Length, _struct$3.Context)), (_struct$4 = onSec, new jquery.JQuery.Ptr(_struct$4.o, _struct$4.Jquery, _struct$4.Selector, _struct$4.Length, _struct$4.Context)), (_struct$5 = offSec, new jquery.JQuery.Ptr(_struct$5.o, _struct$5.Jquery, _struct$5.Selector, _struct$5.Length, _struct$5.Context)), endDate, interval, rounds, roundFinished);
	};
	...
	TabataTimer.Ptr.prototype.tick = function() {
		...
		if (timeDelta > 0) {
			...
			t.element.SetHtml(new Go$String(formattedTime.minutes + ":" + formattedTime.seconds));
		} else {
			if (t.rounds > 0) {
				if (t.roundFinished) {
					jQuery(new (go$sliceType(go$emptyInterface))([new Go$String("#rounds")])).SetVal(new Go$Int(t.rounds));
					t.restart();
				} else {
					t.breakTimer();
	...
	TabataTimer.Ptr.prototype.breakTimer = function() {
		var t, _tuple, offsec, err;
		t = this;
		_tuple = strconv.Atoi(t.offSec.Val()), offsec = _tuple[0], err = _tuple[1];
		if (!(go$interfaceIsEqual(err, null))) {
			return;
		}
		t.endDate = jquery.Now() + offsec * 1000;
		t.roundFinished = true;
		t.element.SetCss(new (go$sliceType(go$emptyInterface))([new Go$String("color"), new Go$String("#FFA100")]));
	};
	TabataTimer.prototype.breakTimer = function() { return this.go$val.breakTimer(); };
	TabataTimer.Ptr.prototype.restart = function() {
		var t, _tuple, onSec, err;
		t = this;
		_tuple = strconv.Atoi(t.onSec.Val()), onSec = _tuple[0], err = _tuple[1];
		if (!(go$interfaceIsEqual(err, null))) {
			return;
		}
		t.endDate = jquery.Now() + onSec * 1000;
		t.roundFinished = false;
		t.element.SetCss(new (go$sliceType(go$emptyInterface))([new Go$String("color"), new Go$String("#0F4DA8")]));
	};
	TabataTimer.prototype.restart = function() { return this.go$val.restart(); };
	TabataTimer.Ptr.prototype.playSound = function() {
		var t;
		t = this;
		go$global.alert(go$externalize("End!", Go$String));
	};
	TabataTimer.prototype.playSound = function() { return this.go$val.playSound(); };
	go$pkg.init = function() {
		(go$ptrType(TabataTimer)).methods = [["breakTimer", "main", [], [], false, -1], ["convertToTime", "main", [Go$Float64], [time], false, -1], ["pad", "main", [Go$Int], [Go$String], false, -1], ["playSound", "main", [], [], false, -1], ["restart", "main", [], [], false, -1], ["start", "main", [], [], false, -1], ["tick", "main", [], [], false, -1]];
		TabataTimer.init([["element", "element", "main", jquery.JQuery, ""], ["onSec", "onSec", "main", jquery.JQuery, ""], ["offSec", "offSec", "main", jquery.JQuery, ""], ["endDate", "endDate", "main", Go$Float64, ""], ["interval", "interval", "main", Go$Int, ""], ["rounds", "rounds", "main", Go$Int, ""], ["roundFinished", "roundFinished", "main", Go$Bool, ""]]);
